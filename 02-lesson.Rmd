...
The code below takes the existing ```co_county_summary_wide``` object, and then uses the ```select``` function the select the columns we want to keep. It then assigns this selection to a new object named "co_county_black_stops": 
...
Note that the name has successfully been changed. 
...
*(Percentage of County Traffic Stops Involving Black Motorists)*-*(Percentage of County's 17-and-over Population that is Black)* 
...
Note that it contains county-level information on the overall population (i.e. all age groups and all racial categories), the  17+ population (for all racial groups), and the Black 17+ population. We can use the latter two variables to compute the second part of our bias index. 
...
Now, when we open ```county_shapefile_biasIndex```, we should see the "bias_index" variable in the dataset, along with the "geometry" information needed to render a map: 
...
Note that these colors are easier to distinguish, which makes it easier to quickly scan the map for relevant patterns.  
...
Let's start by fine-tuning the map of the continuous "bias_index" variable. As an exercise, read through the code below (without scrolling down to the map), and see if you can decipher what each line is doing, forming a mental picture of the in-progres map as you go. If you're having trouble understanding something, look up the function's documentation, which can by accessed by typing the name of the function, preceded by a question mark. For example, if we want to access the documentation for the ```tm_polygons``` function, we can type ```?tm_polygons```, which will bring up the function's documentation (which provides a description of the various arguments to the function), in the "Help" tab on the bottom-right of the R Studio interface. 
...
* The first function that is called is ```tm_shape```; we pass the name of the object that contains the data we want to map to this function ```county_shapefile_biasIndex```, which yields ```tm_shape(county_shapefile_biasIndex)```. 
...
Note that if we'd like to export a dynamic/interactive map, we can use the same code as above; the only difference is that we would use an "html" extension. The interactive map would then be exported to our working directory as an html file, which can then easily be uploaded to a website. 
...
If you're interested in exploring projects resources that lie at the intersection of GIS, Black history, and the study of structural racism, ESRI's page on racial equity is a good place to start: [https://www.esri.com/en-us/racial-equity/overview](https://www.esri.com/en-us/racial-equity/overview). 

If you're interested in exploring a specific example of a large-scale interdisciplinary project on systemic racism that incorporates spatial analysis, check out the [Mapping Prejudice](https://mappingprejudice.umn.edu/) project from the University of Minnesota. 
...
Lovelace, Robin, Jakub Nowosad, and Jannes Muenchow. 2021. Geocomputation With R. https://geocompr.robinlovelace.net/.
...
Wickham, Hadley, Romain Francois, and Lionel Henry. 2020. “dplyr: A Grammar of Data Manipulation.” R Package version 1.0.2. https://CRAN.R-project.org/package=dplyr.
