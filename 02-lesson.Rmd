...
It is also worth reminding ourselves that it's possible to assign the maps we create using *tmap* to objects. For example, let's assign the last map we created to a new object named ```traffic_stop_map_continuous```:
...
If we print the contents of ```traffic_stop_map_categorical```, we open a map that looks something like this:
...
* ```traffic_stop_map_continuous<-``` indicates that the map created through the code on the right side of the assignment operator will be assigned to the object named ```traffic_stop_map_continuous```. This overwrites the previous version of the map assigned to this object.
...
```{r}
traffic_stop_map_continuous_labeled<-
  traffic_stop_map_continuous+
...
```{r}
tmap_save(traffic_stop_map_continuous_labeled, "traffic_stop_map_continuous_labeled.png")
```
...
traffic_stop_map_continuous_labeled<- # object assignment; assigns map to object named "traffic_stop_bias_map"
